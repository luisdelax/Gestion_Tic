...
export default function PrestamosPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const horaPrestamo = formData.horaPrestamo || '12:00:00'
      const horaDevolucion = formData.horaDevolucion || '12:00:00'
      
      const fechaPrestamoLocal = formData.fechaPrestamo ? new Date(formData.fechaPrestamo + 'T' + horaPrestamo) : null
      const fechaDevolucionLocal = formData.fechaDevolucion ? new Date(formData.fechaDevolucion + 'T' + horaDevolucion) : null
  ...
  const selectEquipo = (eq) => {
    ...
    const label = eq.placa ? `${eq.marca} ${eq.modelo} - PLACA: ${eq.placa}` : `${eq.marca} ${eq.modelo} - ${eq.serial}`
    setSearchEquipo(label)
  ...
  const handleEdit = (prestamo) => {
    ...
    if (prestamo.equipoComputo) {
      const eq = prestamo.equipoComputo
      const label = eq.placa ? `${eq.marca} ${eq.modelo} - PLACA: ${eq.placa}` : `${eq.marca} ${eq.modelo} - ${eq.serial}`
      setSearchEquipo(label)
    }
    ...
    setFormData({
      ...
      horaPrestamo: prestamo.fechaPrestamo ? prestamo.fechaPrestamo.split('T')[1]?.substring(0, 5) : '',
      fechaDevolucion: prestamo.fechaDevolucion?.split('T')[0] || '',
      horaDevolucion: prestamo.fechaDevolucion ? prestamo.fechaDevolucion.split('T')[1]?.substring(0, 5) : '',
  ...
  const getEquipoLabel = (prestamo) => {
    if (prestamo.equipoComputo) {
      const eq = prestamo.equipoComputo
      return eq.placa ? `${eq.marca} ${eq.modelo} (PLACA: ${eq.placa})` : `${eq.marca} ${eq.modelo} (${eq.serial})`
    }
  ...
  const columns = [
    ...
    { key: 'fechaPrestamo', header: 'Fecha/Hora Préstamo', render: (val) => val ? formatDateTime(val) : '-' },
    { key: 'fechaDevolucion', header: 'Fecha/Hora Devolución', render: (val) => val ? formatDateTime(val) : '-' },
  ...
  const filteredEquipos = (searchEquipo ? equipos.filter(e => 
        ...
        e.placa?.toLowerCase().includes(searchEquipo.toLowerCase())
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="absolute z-10 w-full mt-1 bg-slate-800 border border-green-500/30 rounded-lg max-h-48 overflow-y-auto">
                  {filteredEquipos.slice(0, 8).map((eq) => (
                    ...
                    >
                      ...
                      <p className="text-xs text-slate-400">{eq.placa ? `PLACA: ${eq.placa}` : `SERIAL: ${eq.serial}`}</p>
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="grid grid-cols-2 gap-2">
              ...
              <Input label="Hora" type="time" value={formData.horaPrestamo} onChange={(e) => setFormData({...formData, horaPrestamo: e.target.value})} required />
            </div>
            <div className="grid grid-cols-2 gap-2">
              ...
              <Input label="Hora" type="time" value={formData.horaDevolucion} onChange={(e) => setFormData({...formData, horaDevolucion: e.target.value})} />
            </div>
          </div>
